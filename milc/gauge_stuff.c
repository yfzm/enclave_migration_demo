...
char ** gen_pt[N_POINTERS];

int  gauge_action_nloops=NLOOP;
int  gauge_action_nreps=NREPS;
int loop_length[NLOOP+1];	/* lengths of various kinds of loops */
int loop_num[NLOOP+1];	/* number of rotations/reflections  for each kind */

    /* table of directions, 1 for each kind of loop */
int loop_ind[NLOOP+1][MAX_LENGTH];
...
int loop_table[NLOOP+1][MAX_NUM][MAX_LENGTH];
    /* table of coefficients in action, for various "representations" (actually,
	powers of the trace) */
double loop_coeff[NLOOP+1][NREPS];
...
int loop_char[MAX_NUM];
	...
	value of the loop */
double loop_expect[NLOOP+1][NREPS][MAX_NUM];
