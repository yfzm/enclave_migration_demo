...
int spec_init () {
    ...
    for (i = 0; i < MAX_SPEC_FD; i++) {
        ...
        printf("[yfzm] try to malloc 0x%lx\n", limit+FUDGE_BUF);
        spec_fd[i].buf = (unsigned char *)malloc(limit+FUDGE_BUF);
        printf("[yfzm] spec_fd[%d].buf: %p\n", i, spec_fd[i].buf);
...
int spec_getc (int fd) {
    ...
    //printf("\n spec_fd: %p, pos: %d\n", spec_fd, spec_fd[fd].pos);
    //printf("spec_fd[fd].buf: %p\n", spec_fd[fd].buf);
...
int spec_fwrite(unsigned char *buf, int size, int num, int fd) {
    ...
    //printf("\n[fwite] spec_fd: %p\n", spec_fd);
    //printf("[fwrite] memcpy %p <- %p with size %lu\n", &(spec_fd[fd].buf[spec_fd[fd].pos]), buf, size*num);
